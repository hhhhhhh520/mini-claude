...
class TestReflectNodeBasic:
    ...
    async def test_reflect_node_simple_task_skips(self):
        ...
        state = create_initial_state(user_input="Fix typo in README", thread_id="test-1")
        state["iteration"] = 1
...
class TestReflectNodeOutput:
    ...
    async def test_reflect_node_returns_correct_keys(self):
        ...
        state = create_initial_state(user_input=complex_task, thread_id="test-3")
        state["iteration"] = 3
...
class TestReflectNodeComplexity:
    ...
    async def test_medium_task_no_reflection(self):
        ...
        state = create_initial_state(
            user_input="Optimize database query for better performance", thread_id="test-medium"
        )
    ...
    async def test_complex_task_reflection(self):
        ...
        state = create_initial_state(user_input=complex_task, thread_id="test-complex")
...
class TestReflectNodeErrorHandling:
    ...
    async def test_llm_json_parse_error(self):
        ...
        state = create_initial_state(user_input=complex_task, thread_id="test-json-error")
    ...
    async def test_llm_connection_error(self):
        ...
        complex_task = "Develop critical production payment system"
        state = create_initial_state(user_input=complex_task, thread_id="test-conn-error")
    ...
    async def test_llm_timeout_error(self):
        ...
        complex_task = "Develop critical production payment system"
        state = create_initial_state(user_input=complex_task, thread_id="test-timeout")
...
class TestReflectNodeToolAnalysis:
    ...
    async def test_reflect_with_tool_calls(self):
        ...
        state = create_initial_state(user_input=complex_task, thread_id="test-tools")
        ...
        result_msg = HumanMessage(
            content="Tool write_file result: File created successfully", name="write_file"
        )
    ...
    async def test_reflect_with_failed_tools(self):
        ...
        complex_task = "Develop payment system for production"
        state = create_initial_state(user_input=complex_task, thread_id="test-failed-tools")

        # Add failed tool call
        error_msg = HumanMessage(
            content="Tool write_file error: Permission denied", name="write_file"
        )
